...
class MutationDescriptionGenerator:
    ...
    def _generate_inframe_desc(
        ...
        if c_del_match and p_del_match:
            ...
            return (
                ...
                f"第{c_start}位至第{c_end}位核苷酸缺失，"
                f"导致相应蛋白序列中第{p_pos1}位{ref1_name}至第{p_pos2}位{ref2_name}共{del_count}个氨基酸缺失，"
                f"此突变在样本中的突变丰度为{frequency:.2f}%。"
            )
    ...
    def generate_cnv_desc(
        ...
        Args:
            gene: 基因名称
            cnv_type: CNV类型 ("Amp"=扩增, "Del"=缺失, "Gain"=增加, "Loss"=减少)
            copy_number: 拷贝数值（可选）
        ...
        type_cn = type_map.get(cnv_type, cnv_type)

        if copy_number is not None:
            return f"该样本检出{gene}基因{type_cn}，拷贝数为{copy_number:.1f}。"
        return f"该样本检出{gene}基因{type_cn}。"

    def generate_fusion_desc(
        self,
        gene1: str,
        gene2: str,
        breakpoint1: Optional[str] = None,
        breakpoint2: Optional[str] = None,
        ...
        Args:
            gene1: 第一个基因（5'端）
            gene2: 第二个基因（3'端）
            breakpoint1: 第一个基因的断点位置（如 "exon 13"）
            breakpoint2: 第二个基因的断点位置（如 "exon 20"）
            frequency: 融合频率（%）
        ...
        desc = f"该样本检出{fusion_name}融合基因"

        if breakpoint1 and breakpoint2:
            desc += f"，断点位于{gene1} {breakpoint1}和{gene2} {breakpoint2}"

        if frequency is not None:
            desc += f"，融合频率为{frequency:.2f}%"

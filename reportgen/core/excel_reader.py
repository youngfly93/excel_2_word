...
class ExcelReader:
    ...
    def __init__(
        self, config_dir: str = "config", log_file: Optional[str] = None, log_level: str = "INFO"
    ):
        """
        初始化Excel读取器
        ...
        self.dtype_inference = bool(self.config_loader.get_setting("data.excel.dtype_inference", True))
        self.keep_default_na = bool(self.config_loader.get_setting("data.excel.keep_default_na", True))
    ...
    def _collect_needed_sheets(self) -> tuple[set[str], bool]:
        table_data = self.mapping_config.get("table_data", {}) if self.mapping_config else {}
    ...
    def _extract_tmb_value(self, df: pd.DataFrame) -> Optional[float]:
        ...
        if tcga_row is not None:
            ...
            if header_idx is not None:
                ...
                tmb_col = next((j for j, h in enumerate(header) if h.upper() == "TMB"), None)
                sampletp_col = next(
                ...
                for i in range(header_idx + 1, len(values)):
                    if is_empty_row(values[i]):
                        break
                    # 新块标题（通常只有第1列非空）
                    if isinstance(values[i][0], str) and values[i][0].strip() and try_float(values[i][0]) is None:
                        if sum(
                            1
                            for v in values[i]
        ...
        for i in range(len(values)):
            if row_has_token(values[i], "Var_num") and row_has_token(values[i], "TMB"):
                ...
                for j in range(i + 1, min(i + 5, len(values))):
                    ...
                    for k in range(values[j].shape[0] - 1, -1, -1):
            ...
            for meta_sheet in ("Meta", "BasicInfo", "基本信息"):
                if meta_sheet in sheet_names:
                    try:
                        ...
                        self.logger.warning("读取Meta信息失败", sheet=meta_sheet, error=str(e))

            # ✅ 提取TMB值（如果TMB sheet存在）
            if "TMB" in sheet_names:
                # 以 header=None 读取整表，便于在多个“块”里定位正确的TMB来源
                tmb_df = self._parse_sheet(excel_file, "TMB", sheet_cache, skip_rows=0, header=None)
                ...
                if msi_status is not None:
                    ...
                    if msi_percentage is not None:
                        try:
                            ...
                            self.logger.info("根据百分比判定MSI状态", percentage=pct, status=status)
                        except (ValueError, TypeError):
                            self.logger.warning("无法解析MSI百分比", value=msi_percentage)
            ...
            for sheet_name in sheets_to_read:
                ...
                if skip_rows > 0:
                    df = self._parse_sheet(
                        excel_file, sheet_name, sheet_cache, skip_rows=skip_rows, header=0
                    )
                    self.logger.debug(
                        "读取Sheet（跳过行）",
                        sheet=sheet_name,
    ...
    def _extract_meta_values(self, sheet_name: str, df: pd.DataFrame, data_source: ExcelDataSource) -> None:
        """
        ...
        if len(df) == 1:
            ...
            for column_name, value in row.items():
                if pd.isna(value):
                    continue
                data_source.add_single_value(str(column_name), self._convert_to_python_type(value))
            self.logger.info("从Meta表(横向)提取单值字段", sheet=sheet_name, count=len(df.columns))
            return
        ...
        if key_idx is not None and val_idx is not None and key_idx != val_idx:
            ...
            for _, r in df.iterrows():
                ...
                if pd.isna(k) or (isinstance(k, str) and not k.strip()):
                    continue
                data_source.add_single_value(str(k).strip(), self._convert_to_python_type(v))
                cnt += 1
            self.logger.info("从Meta表(Key/Value)提取单值字段", sheet=sheet_name, count=cnt)
            return
        ...
        if df.shape[1] == 2:
            ...
            for _, r in df.iterrows():
                ...
                data_source.add_single_value(str(k).strip(), self._convert_to_python_type(v))
                cnt += 1
            self.logger.info("从Meta表(两列无表头)提取单值字段", sheet=sheet_name, count=cnt)
            return

    def _extract_single_values(self, df: pd.DataFrame, data_source: ExcelDataSource) -> None:
        """
        ...
        self.logger.debug("提取单值字段", count=len([v for v in first_row if not pd.isna(v)]))
        ...
        try:
            ...
            for i in range(len(df_raw)):
                ...
                if first_col.startswith("HLA-"):
                    ...
                    type1_qc = None
                    type2_qc = None
                    self.logger.debug("发现HLA位点", locus=current_locus, zygosity=second_col)
                ...
                # 检测Type 1
                    ...
                    if current_item:
                        ...
                        for col_idx in range(2, min(6, len(df_raw.columns))):
                            qc_str = str(df_raw.iloc[i, col_idx]) if pd.notna(df_raw.iloc[i, col_idx]) else ""
                        ...
                        self.logger.debug("发现Type 1", locus=current_locus, allele=second_col, qc_exons=list(type1_qc.keys()))
                ...
                # 检测Type 2
                    ...
                    if current_item:
                        ...
                        self.logger.debug("发现Type 2", locus=current_locus, allele=second_col, qc_exons=list(type2_qc.keys()))
    ...
    def read_sheet(self, file_path: str, sheet_name: str) -> pd.DataFrame:
        ...
        try:
            ...
            if skip_rows > 0:
                ...
                df = pd.read_excel(file_path, sheet_name=sheet_name, engine="openpyxl")
                self.logger.debug("读取Sheet", file=file_path, sheet=sheet_name, rows=len(df))
            return df
        except Exception as e:
            self.logger.error("读取Sheet失败", file=file_path, sheet=sheet_name, error=str(e))
    ...
    def _extract_sample_id_from_filename(self, file_path: str) -> Optional[str]:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                self.logger.debug("从文件名提取样本编号", filename=filename, sample_id=sample_id)
                return sample_id

        # 如果所有模式都不匹配，尝试提取下划线分隔的第一部分
        parts = filename.split("_")
        if parts:
            first_part = parts[0]
            if len(first_part) >= 6:  # 样本编号至少6个字符
                self.logger.debug(
                    "使用文件名第一部分作为样本编号", filename=filename, sample_id=first_part
                )
                return first_part

...
class DataCleaner:
    ...
    def normalize_date(self, date_value: Any, output_format: str = "%Y-%m-%d") -> Optional[str]:
        """
        标准化日期格式
    ...
    def format_number(self, number_value: Any, decimal_places: int = 2, unit: str = "") -> str:
        """
        格式化数值
    ...
    def _validate_required_fields(self, report_data: ReportData) -> None:
        ...
        for field_name in required_fields:
            ...
            if value is None or (isinstance(value, str) and value == ""):
                # 不要在验证失败时覆盖字段值，只记录错误
                self.logger.warning("必填字段缺失或为空", field=field_name, current_value=value)

...
class ProjectDetector:
    ...
    def __init__(
        ...
        self.config = config_loader.load_project_types_config()
    ...
    def _calculate_match_score(
        ...
        if keyword_groups:
            return self._calculate_group_match_score(project_type, filename, detection_text)
        ...
        if matched_in_filename:
            details.append(f"文件名匹配关键词: {', '.join(sorted(matched_in_filename))}")
    ...
    def _match_keyword_group(
        ...
        *,
        ...
        if isinstance(group, list):
            ...
            return False, [f"组{group_index}: 配置格式无效({type(group).__name__})"], 0.0
        ...
        if any_patterns:
            if matched_any:
                details.append(
                    f"组{group_index}: 任一匹配({', '.join(matched_any)}) [{matched_any_where}]"
                )
            else:
        ...
        if all_patterns:
            if matched_all:
                details.append(
                    f"组{group_index}: 全部匹配 [{matched_all_where}]"
                )
            else:
    ...
    def _pattern_matches(self, pattern: KeywordPattern, text: str, *, case_sensitive: bool) -> bool:
        if not text:
            return False
        ...
        *,
        ...
        for pattern in patterns:
            ...
            if (
                "content" in sources
                and detection_text
                and self._pattern_matches(pattern, detection_text, case_sensitive=case_sensitive)
            ):
                matched.append(disp)
                where.add("content")
    ...
    def _extract_detection_text(self, excel_data: Optional[Any]) -> Optional[str]:
        ...
        if detection_field:
            field_cfg = (self.mapping_config.get("single_values", {}) or {}).get(detection_field, {})
            synonyms = field_cfg.get("synonyms", []) if isinstance(field_cfg, dict) else []

...
def validate_excel_file(file_path: str) -> tuple[bool, Optional[str]]:
    ...
    """
    # 验证基本路径和扩展名
    is_valid, error = validate_file_path(file_path, must_exist=True, file_extensions=[".xlsx"])
    if not is_valid:
        return False, error
...
def validate_docx_file(file_path: str, must_exist: bool = True) -> tuple[bool, Optional[str]]:
    """
    验证Docx文件
    ...
    Returns:
        (是否有效, 错误消息)
    """
    return validate_file_path(file_path, must_exist=must_exist, file_extensions=[".docx"])
    ...
    if data_type == "string":
        ...
        try:
            ...
            if min_value is not None and float_value < min_value:
                return False, f"字段 {field_name} 值 {float_value} 小于最小值 {min_value}"
            if max_value is not None and float_value > max_value:
                return False, f"字段 {field_name} 值 {float_value} 大于最大值 {max_value}"
        except (ValueError, TypeError):
